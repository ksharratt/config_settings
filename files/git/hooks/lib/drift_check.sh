...
git fetch --quiet "$REMOTE" "$BASE_BRANCH" || echo "WARN: fetch failed; using cached origin/master" >&2
...
echo "------------------------------------------------------------"
echo "WARNING (pre-commit): '$branch' is behind ${REMOTE}/${BASE_BRANCH}"
...
echo "Commits on ${REMOTE}/${BASE_BRANCH} not in '$branch':"
git log --oneline --decorate "${head_sha}..${base_sha}" || true
echo

# File-level differences
echo "Files changed between ${REMOTE}/${BASE_BRANCH} and '$branch':"
git diff --name-status "${base_sha}..${head_sha}" || true
echo

# Show patch for what you're missing from origin/master (often the most useful)
echo "Patch you are missing from ${REMOTE}/${BASE_BRANCH} (first 250 lines):"
git diff "${head_sha}..${base_sha}" | sed -n '1,250p' || true
echo

echo "Suggested next step (choose one):"
echo "  Merge:  git merge ${REMOTE}/${BASE_BRANCH}"
echo "  Rebase: git rebase ${REMOTE}/${BASE_BRANCH}"
echo "------------------------------------------------------------"
